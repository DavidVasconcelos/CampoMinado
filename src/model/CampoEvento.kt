...
data class Campo(val linha: Int, val coluna: Int) {
    ...
    private val callbacks = arrayListOf<(Campo, CampoEvento) -> Unit>() //recebe uma função como parâmetro
    ...
    fun abrir() {
        if (fechado) {
            ...
            if (minado) {
                ...
                vizinhos.filter { it.fechado && it.seguro && this.vizinhancaSegura }.forEach { it.abrir() }
            }
        }
    }
